var cols,rows,//כמות עמודות וטורים
    ...
    time,//הזמן שיש לשלב
    roundTime=30,//זמן שנוסף בכל שלב
...
function setup() {
  ...
  diff=0;
  difficulty=[width/4,width/5,width/6,width/7,width/8];
...
function draw() { 
  ...
  if(touches.length===1&&mouseY>space){//מזיז את השחקן לפי הטאצ
    touchPlay();
    open();
  }
...
function open(){
  grid[index(player.i,player.j)].open=true;
}
